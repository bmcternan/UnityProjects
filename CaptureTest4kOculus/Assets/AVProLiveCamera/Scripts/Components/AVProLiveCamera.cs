...
public class AVProLiveCamera : MonoBehaviour
{
    ...
    //public SelectModeBy _modeSelection = SelectModeBy.Resolution;
    ...
    public string _desiredFormat = "MJPG";

    //private const int DEVICE_NUMBER = 7;
    //private const int MODE_NUMBER = 31;
    private const int DEVICE_NUMBER = 0;
    private const int MODE_NUMBER = 31 ;
    private const bool USE_PRESELECTION = true;
    //private const bool USE_PRESELECTION = false;
    //private const bool SHOW_MODES = true;
    private const bool SHOW_MODES = false;
    ...
    void Reset()
    {
        ...
        //_modeSelection = SelectModeBy.Resolution;
    ...
    public void SelectDeviceAndMode()
    {
        ...
        Debug.Log("SelectDeviceAndMode");
        if (USE_PRESELECTION)
        {
            _deviceSelection = SelectDeviceBy.Index;
            _desiredDeviceIndex = DEVICE_NUMBER;
            _modeSelection = SelectModeBy.Index;
            _desiredModeIndex = MODE_NUMBER;
        }
    ...
    private AVProLiveCameraDeviceMode SelectMode()
    {
        ...
        Debug.Log("SelectMode " + _modeSelection);
        ...
        switch (_modeSelection)
        {
            ...
            case SelectModeBy.Resolution:
                ...
                {
                    result = GetClosestMode(_device, _desiredResolutions, _maintainAspectRatio, _desiredFormat);
                    if (result == null)
                    {
            ...
            case SelectModeBy.Index:
                Debug.Log("SelectMode index " + _desiredModeIndex);
                if (SHOW_MODES)
                {
                    for (int i = 0; i < _device.NumModes; i++)
                    {
                        result = _device.GetMode(i);
                        Debug.Log("Index check mode " + i + " " + result.Width + "x" + result.Height + " fps " + result.FPS + " " + result.Format);
                    }
                }
                if (USE_PRESELECTION)
                {
                ...
                if (_desiredModeIndex >= 0)
                {
                    ...
                    if (result == null)
                    {
                        ...
                        Debug.Log("BJM setting mode to " + result.Width + "x" + result.Height + " fps " + result.FPS + " format \"" + result.Format.ToString() + "\"");
                }
                break;
        }
    ...
    {
        Debug.Log("SelectDevice " + _deviceSelection.ToString());
        ...
        switch (_deviceSelection)
        {
            ...
            case SelectDeviceBy.Index:
                Debug.Log("SelectDevice Index " + _desiredDeviceIndex);
    ...
    private static AVProLiveCameraDeviceMode GetClosestMode(AVProLiveCameraDevice device, List<Vector2> resolutions, bool maintainApectRatio, string desiredFormat)
    {
        AVProLiveCameraDeviceMode result = null;

        Debug.Log("GetClosestMode with " + resolutions[0].x + "x" + resolutions[0].y + " format \"" + desiredFormat + "\"");
        ...
        {
            result = device.GetMode(i);
            Debug.Log("mode " + i + " " + result.Width + "x" + result.Height + " fps " + result.FPS + " " + result.Format);

            if ((result.Width == resolutions[0].x) && (result.Height == resolutions[0].y) && (result.Format == desiredFormat))
                break;
        }
        return result;
    }
